...
{
    SVCERR << "PLAYSPEED CHANGED " << position << endl;
...
int JackAudioCallbackAdapter(jack_nframes_t nframes, void* obj) {
    return static_cast<MIDIBeatLoopThread*>(obj)->process(nframes);
}

int MIDIBeatLoopThread::process(jack_nframes_t nframes)
{
    ...
    if (this->m_playSource->isPlaying()) {
        m_stopSended = false;
        if (m_events.size() == 0)
            m_events = m_beatModel->getAllEvents();

        sv_frame_t currentFrame = m_playSource->getCurrentBufferedFrame();
        double timeStretch = m_playSource->getTimeStretch();

        // DUPLICATE CODE... send fills
        for (auto it = m_fill24.begin(); it != m_fill24.end(); ++it) {
            if (it->getFrame() >= currentFrame
                    && it->getFrame() < currentFrame + nframes / timeStretch) {
                //SVCERR << currentFrame << " fillBEAT" << endl;

                sv_frame_t position = it->getFrame() - currentFrame;
                position *= timeStretch;
                //send_rt_message(port_buf, position, MIDI_RT_CLOCK);
                eventsToSend.push_back(MidiEvent(position, MIDI_RT_CLOCK));
                m_fill24.erase(it--);
            }
        }


        // TODO don't iterate all events
        for (size_t i = 0; i < m_events.size(); ++i) {
            auto ev = m_events.at(i);
            // new beat
            if (ev.getFrame() >= currentFrame
                    && ev.getFrame() < currentFrame + nframes / timeStretch) {

                if (!m_clockInitialised) {
                    const int64_t bcnt = 0;

                    jack_midi_data_t *buffer;
                    buffer = jack_midi_event_reserve(port_buf, 0, 3);

                    buffer[0] = MIDI_SONG_POSITION;
                    buffer[1] = (bcnt)&0x7f; // LSB
                    buffer[2] = (bcnt>>7)&0x7f; // MSB

                    SVCERR << "MIDI_START" << endl;
                    //send_rt_message(port_buf, 0, MIDI_RT_START);
                    eventsToSend.push_back(MidiEvent(3, MIDI_RT_START));

                    m_clockInitialised = true;
                }

                SVCERR << currentFrame << " BEAT" << endl;

                sv_frame_t position = ev.getFrame() - currentFrame;
                position *= timeStretch;
                // send_rt_message(port_buf, position, MIDI_RT_CLOCK);
                eventsToSend.push_back(MidiEvent(position, MIDI_RT_CLOCK));

                // Quick Hack, send the remaining of the 24Beats to stay in sync
                for (size_t k = 0; k < m_fill24.size(); k++) {
                    //send_rt_message(port_buf, position++, MIDI_RT_CLOCK);
                    eventsToSend.push_back(MidiEvent(position++, MIDI_RT_CLOCK));
                m_fill24.clear();

                if (i < m_events.size() - 1) {
                    //SVCERR << "createFill24" << endl;
                    auto nextEvent = m_events[i + 1];
                    auto duration = nextEvent.getFrame() - ev.getFrame();

                    sv_frame_t positionQuant = std::round(duration / 24);
                    for (int j = 1; j < 24; ++j) {
                        m_fill24.push_back(Event(ev.getFrame() + j * positionQuant));
                    }
                }
             }
        }
        ...
        /*std::remove_if(m_fill24.begin(), m_fill24.end(), [](Event const &ev) {
            return ev.getFrame() < currentFrame;
        });*/

    } else {
        m_clockInitialised = false;
        if (!m_stopSended) {
            SVCERR << "MIDI STOP" << endl;
            // send_rt_message(port_buf, 0, MIDI_RT_STOP);
            m_events.clear();
            eventsToSend.clear();
            eventsToSend.push_back(MidiEvent(0, MIDI_RT_STOP));
            m_stopSended = true;
        }
    }

    std::sort(eventsToSend.begin(), eventsToSend.end(), [](MidiEvent ev1, MidiEvent ev2) {return ev1.position < ev2.position;});

    std::for_each(eventsToSend.begin(), eventsToSend.end(), [&port_buf](MidiEvent ev) {send_rt_message(port_buf, ev.position, ev.message);});

    // FIXME quick hack, ignore 24 for test
    // eventsToSend.clear();

    return 0;
}

void MIDIBeatLoopThread::fillUpEvents24Times() {
    m_events = m_beatModel->getAllEvents();

    EventVector newEvents;
    ...
    for (size_t i = 0; i < m_events.size(); ++i) {
        ...
        if (i < m_events.size() - 1) {
            ...
            auto positionQuant = std::round(duration / 24);

            for (int j = 1; j < 24; ++j) {
                newEvents.push_back(Event(ev.getFrame() + j * positionQuant));
            }
        }
    }

    SVCERR << "event size: " << m_events.size() << endl;
    SVCERR << "new events size: " << newEvents.size() << endl;

    m_events = newEvents;
}

void MIDIBeatLoopThread::run() {
...
//    while (true) {
//        if (this->m_playSource->isPlaying()) {
//            sv_frame_t currentFrame = m_playSource->getCurrentPlayingFrame();
//            // TODO don't iterate all events
//            EventVector events = m_beatModel->getAllEvents();
...
{
    if (ModelById::isa<SparseOneDimensionalModel>(modelId)) {
        SVCERR << "Model added " << endl;
        MIDIBeatLoopThread *mlThread = new MIDIBeatLoopThread();
        mlThread->m_beatModel = ModelById::getAs<SparseOneDimensionalModel>(modelId);
        mlThread->m_playSource = this->m_playSource;

        // TODO stop thread when removed
        mlThread->start();
    }
